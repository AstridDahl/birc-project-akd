...
for sp,name in sp_name_dict.items():
	...
	if name == 'bonobo': in_path = f'{in_path}/chimpanzee/ref'
	else: in_path = f'{in_path}/{name}/ref'
	...
	for sample in sp_sample_dict[sp]:
		...
		if sample == 'Carl': barcode_file = path_to_barcodes_v3
		else: barcode_file = path_to_barcodes_v2
